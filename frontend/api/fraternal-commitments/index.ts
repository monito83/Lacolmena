...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    if (req.method === 'GET') {
      // Obtener compromisos fraternos
      const { search, family_id, student_id, academic_year, status, page = '1', limit = '50' } = req.query;
      ...
      // Actualizar compromiso fraterno
      const { id } = req.query;
      const {
        academic_year,
        agreed_amount,
      ...
      // Finalizar compromiso fraterno (soft delete)
      const { id } = req.query;
